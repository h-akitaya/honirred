...
#           Ver 2.01 ?
#           Ver 2.10 2013/12/24 H.Akitaya : moving object
#           Ver 2.11 2014/07/22 H.Akitaya : bug fix for mag0
#           Ver 2.11 2018/08/06 H.Akitaya : bug fix for printf for apertures
#
...
real epadu=2.2 # CCD
bool interactive=no
bool movingobj=no   # moving object mode
real mjd0 = 0.0   # base MJD
real dxdt = 0.0   # dx/dt (pixels/day)
real dydt = 0.0   # dy/dt (pixels/day)
bool countonly=no # output count only
...
#     tmpmag_fn = "tmp$tmp_qlphottmpmag.mag"
     ...
     imgfiles = mktemp( "tmp$tmp_qlphot_1_" )
...
#	hselect( img, "LST", yes) | scanf("\"%d/%d/%d %d:%d:%d\"", year, month, day, hh, mm, ss)
	...
	if( interactive==yes || offset_fn !="" || movingobj==yes){
	    tmp_coord=mktemp ( "tmp$tmp_qlphot_2_" )
	    display( img, 1, >& "dev$null" )
	    if( interactive == yes ) {
  	       getimcur | scanf( "%f %f %s", star0_x, star0_y, buf1 )
            } else if ( movingobj == yes ){
	       star0_x = 0.0
	       star0_y = 0.0
            } else {
	    ...
	    while( fscanf( list, "%f %f", tmp_x, tmp_y ) != EOF ){
	        if( i==0 ) {
		   diff_x = star0_x - tmp_x
		   diff_y = star0_y - tmp_y
                   if( movingobj == yes ){
                       diff_x = ( mjd-mjd0 ) * dxdt
                       diff_y = ( mjd-mjd0 ) * dydt
		   }
		} else {
		   diff_x = 0
		   diff_y = 0
...
#	    type( tmp_coord )
	...
	qphot( img, cbox, annulus, dannulus, apertures, inter-, coords=coord_fn_, output=tmpmag_fn, epadu=epadu, zmag=zmag, filter="WH_IRAF2" )
