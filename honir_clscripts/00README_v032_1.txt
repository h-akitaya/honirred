...
* 初期設定
 ...
 6. 標準キャリブデータを設定する
   ...
   データセットの名前を聞かれたら、201404a (ないし、現在はデフォルトが
  そうなっているのでそのまま enter)とする。引数で与えても良い。

   honir> honirinit
   Keycode for observational run to be reduced (201402a|201404a|xxxxxx) \
                    (201404a): [enter]
   ...
   #Done
 ...
 ※ 自分で作成したキャリブデータを登録したいときは、honir_cal/に置いて、
    honirinit.cl内を適当に修正すること。
...
(4) flat処理
 ...
 ecl>  hnflatten HN*ira*ds.fits object="KISS14k" auto+

        HN*ira*ds.fits のうち、KISS14kの画像のみを選び、登録してある
       適切なflat画像を持ってきてフラット割りする。
      ※ 分光、偏光撮像のフラットは未登録。各自作成して、honirinit.cl に
         記述しておくこと。
...
* pipeline 処理 (2014/4/17-)

(1) 必要データ自動取得

  python (path)/honir_prog/gethonirfile.py 20140317 KISS14k Imaging
     20140317 : データ保存ディレクトリの名前(YYYYMMDD)
     KISS14k: 天体の名前(FITS headerの "OBJECT"の内容)
     Imaging : 観測種類 (FITS headerの "OBS-MODE" )
     
  ※ gethonirfile.py 内、データ置き場の設定(BASE_DIR)は要設定。
  例えば、work5の場合は、
     BASE_DIR = "/mnt/data3/honir"
  とする。

(2) 一次処理
  $ cl
  ecl> honir
  ecl> honirinit 201406
  ecl> hnpreproc KISS14k opt+ ira+ partial-
 
    hnpreproc (天体名=fits headerの"OBJECT"の値)
      で、特定の天体のデータの一次処理 (flatteningまで)を行う。

      ※ 分光、偏光撮像のフラットは未登録。各自作成して、honirinit.cl に
         記述しておくこと。
    opt+/- : 可視画像を対象とする場合、opt+。除外する場合 opt-
    ira+/- : 近赤外VIRGO画像を対象とする場合、opt+。除外する場合 opt-
    partial+/- : 可視画像が部分読み出しのときはpartial+。
    	         全域読み出しの場合はpartial-。

(3-a) 撮像sky引き・stacking

  hnskycomb HN*ira*_fl.fits sky_j.fits over+ scale+ filter="J" object="KISS14k"

  hnskysub HN*_fl.fits skyfn="sky_j.fits" filter="J" \
    scaling+ object="KISS14k"

  hnstack HN*ira*sk.fits kiss14k_j_20140317.fits object="KISS14k" \
    filter="J" ref=""
      - ds9を立ち上げておくこと。
      - 画像を表示するレンジ(下限と上限)を聞かれるので、それぞれ答える。
      - その後、画像あわせに使う星の位置を、"m"キーでマーク(複数可)。
      　その後、"q"キーで抜けると、stackingが実行される。

(3-b) 偏光分光
  hnspecpol1 HD114710 opt filter="none" nod="ABA" extn="_noreid" reident-
   (詳細は秋田谷まで問い合わせを)
 ...
 ** 大きめの宇宙線の手動除去ツール: eracecosray.cl

   honir> erasecosray HN1408010001ira00_fl.fits over+ interactive+
      over+ : 上書きOKの場合。
      interactive+: インタラクティブモード(通常はこちらを使う)

 ** ds9への画像表示

   honir> simg HN1408010001ira00_fl.fits
      IRAF displayタスクではなく、xpasetを用いた表示。
      ~/iraf/honir_prog/simg が必要。
