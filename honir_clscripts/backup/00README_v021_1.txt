...
* 初期設定
 ...
 5. 標準キャリブデータを設定する
   ...
   データセットの名前を聞かれたら、201402a (ないし、デフォルトが
  そうなっているのでそのまま enter)とする。
...
EXAMPLES
  ...
  1. 撮像フラット画像の作成。0.2より小さいカウントのピクセルは1に置き換え。
     標準キャリブデータにあるmaskを使ってbad pix補正。
        cl> hnmkflatvirgo flat_j.fits HN1401290047ira??.fits \
            HN1401290051ira??.fits \
...
(0) ここの task全体の共通事項
 ...
 parametersに
   subext_in, subext_out (入力・出力ファイルのサブ拡張子)がデフォルト
   指定してある。
   たとえば、hndsubvirgo の場合は、subext_in="_bt", subext_out="ds"。
   これは、入力ファイルが"XXXXXXXX_bt.fits"であり、出力ファイルを
   "XXXXXXXX_ds.fits"にすることを意味する。
   もし、入力ファイルの末尾が異なる場合は、適宜、subext_inを指定しなおす
   こと。
    e.g.) サブ拡張子がないファイル名の画像(YYYYYYY.fits)に対して
          hndsubvirgo をかけるときは、subext_in=""とする。
   また、出力ファイルのサブ拡張子を変えたいときは、subext_outを変える。
...
(1) reference pixel の削除、trimming
  ...
  ecl> hntrimvirgo HN*ira??.fits
   ...
   - その他の便利なparameters
　　-- 既存ファイルを上書きするとき
      hntrimvirgo *fits over+

(2) dark画像の作成

  ecl> hnmkdark *_bt.fits darkave060.fits etsel+ dksel+ expt=60 \
       darklst="dark.lst"
...
(3) dark 差し引き
  ...
  ecl> hndsubvirgo *_bt.fits darklst="dark.lst" objsel+ \
       object="KISS14g"
...
(4) flat処理
 ...
 ecl>  hnflatten *ds.fits flat="flat_img_j.fits" fltrsel+ filter="J"

         *_ds.fits について、filterが"J"の画像のみを選び
         (fltrsel+ filter="J")、flat_img_j.fits のフラット画像で
         割る( flat="flat_img_j.fits" )。
	 出力は *_fl.fits になる。

   - その他の便利なparameters
     -- 入力画像リストから、FITS header OBJECTが特定の画像のみを選ぶとき
        objsel+ object="KISS14g"
         
(5) bad pix correction
 
 ecl> hnbpfixvirgo *_fl.fits calbp+ calds+ objsel+ object="SZ_Cam"
      over+
   ...
   - その他の便利なparameters
     -- bad pixel mask, dark spot maskを自分で指定したいとき。
         calbp- bpmask="badpixmask.pl"
              and/or
         calds- dsmask="darkspotmask.pl"
    -- 特定のfilterの画像のみ指定するとき
         fltrsel+ filter="J"

(6) sky合成 (dithering した画像に対して)
   
  ecl> hnskycomb HN*_fl.fits sky_j.fits over+ scale+ fltrsel+ filter="J"
