...
#           Ver 1.13 2014/08/01 H.Akitaya : fits header addition
#           Ver 1.14 2014/10/28 H.Akitaya : rejection procedure option
#
...
string se_out = "_tr"
string mainext = ".fits"
string reject = "sigclip" { prompt = "Rejection Procedure" }
real boxsize=25
real bigbox=31
...
real mjd_cen, exp_tot, rdn_sng, rdnoise
int grpno, nimg
real x_cur, y_cur
bool objsel = no
bool fltrsel = no
...
if ( nimg != 0 ){
	...
	reject=reject, lsigma=lsigma, hsigma=hsigma )
	# calc MJD-CENTER
	hselect(  "@"//imgfiles_out, "MJD", yes) | average | scanf("%f %f %i", mjd_cen, x, i )
	printf("%20.8f\n", mjd_cen ) | scanf("%s", s1 )
	printf("MJD-CEN: %s\n", s1 )
	hedit( output_fn, "MJD-CEN", ""//s1//"", add+, addonly+, delete+, verify-,\
	       show-, update+ )

	# header information addition
	hedit( output_fn, "HNSTACK", yes, add+, verify-, show- )
	# calc total exposure time and effective readout noise
	hselect(  "@"//imgfiles_out, "EXPTIME0", yes) | average | scanf("%f %f %i", x, y, i )
	exp_tot = x*i
	printf("%20.2f\n", exp_tot ) | scanf( "%s", s1 )
	printf("EXP-TOT: %s\n", s1 )
	hedit( output_fn, "EXP-TOT", ""//s1//"", add+, addonly+, delete+, verify-,\
	       show-, update+ )
	hselect(  "@"//imgfiles_out, "RDNOISE", yes) | average | scanf("%f %f %i", rdnoise, y, i )
	rdn_sng = sqrt(i)*rdnoise
	printf("%20.3f\n", rdn_sng ) | scanf( "%s", s1 )
	printf("RDN-SNG: %s\n", s1 )
	hedit( output_fn, "RDN-SNG", ""//s1//"", add+, addonly+, delete+, verify-,\
	       show-, update+ )
    }
    if( preserve == no )
        imdelete( "@"//imgfiles_out, ver- )
