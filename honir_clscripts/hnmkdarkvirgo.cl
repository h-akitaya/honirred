...
bool dksel = yes          # auto selection for "DATA-TYP = DARK"
...
string selection_flg, datatyp, hnarm, trimed
...
while( fscan( imglist, img ) != EOF ){
  hselect( img, "HN-ARM", yes ) | scanf("%s", hnarm )
  hselect( img, "HNTRIMV", yes, missing="INDEF" ) | scanf("%s", trimed )
  if( hnarm != "ira" || trimed != "yes" ){
      printf("File %s is not a trimed VIRGO image. Skip.\n", img )
      next
  }
