...
class Photometry( object ):
    ...
    def calcArea( self, diameter ):
        area = math.pi * ( diameter * self.pixscale / 2.0 )**2
        return area
    def getSN( self, objsignal, bjfluc, diameter ):
        sn = objsignal*self.gain / \
             math.sqrt( (bjfluc*self.gain)**2*self.calcArea( diameter )+ \
                        objsignal*self.gain)
        return sn
    ...
    def calcObjSignal( self, sn, bjfluc, diameter ):
        objsignal = 1.0/self.gain/2.0 * \
                    (sn**2+ math.sqrt( sn**4 + 4.0*(bjfluc*self.gain)**2*\
                                       self.calcArea( diameter ) * 
                                       sn**2 ) )
        return objsignal
...
if __name__ == '__main__':
    ...
    sn = float( sys.argv[4] )
    print "Aperture Diameter [arcsec]:", diameter
    print "Required Signal [ADU]:", ph.calcObjSignal( sn, bjfluc, diameter )
    print "Limiting Magnitude [mag]:", ph.calcLimMag( sn, bjfluc, diameter )
