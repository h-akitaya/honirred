...
float linearfunc( float x, float x1, float x2, float y1, float y2 );
...
int bpfix( long y, long x_begin, long x_end, fits_image &image ){
...
#endif
  long x, y1, y2;
  y1 = image.dvalue( x_begin-1, y );
  y2 = image.dvalue( x_end+1, y );
  for( x=x_begin; x<=x_end; x++){
    if( x_begin == 1 )
      ...
      image.assign( linearfunc( x, x_begin-1, x_end+1, y1, y2 ), x, y);
  }
  return 0;
}
